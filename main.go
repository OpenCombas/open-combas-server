	...
	for _, serverConfig := range cfg.Servers {
		if serverConfig.Enabled {
			...
			case config.SquadReg:
				...
				go srv.Run()
			case config.SquadJoin:
				srv := server.NewSquadJoinServer(address, serverConfig, cfg.DefaultBufferSize, &cfg.Logging, ctx, &wg, cfg.Prometheus, prometheus.WrapRegistererWith(prometheus.Labels{"server_type": string(serverConfig.Type), "server_name": string(serverConfig.Label)}, reg), squadRepo)
				go srv.Run()
			...
			case config.SquadRanking:
				...
				go srv.Run()
			case config.SquadHistory:
				srv := server.NewSquadHistoryServer(address, serverConfig, cfg.DefaultBufferSize, &cfg.Logging, ctx, &wg, cfg.Prometheus, prometheus.WrapRegistererWith(prometheus.Labels{"server_type": string(serverConfig.Type), "server_name": string(serverConfig.Label)}, reg), squadRepo)
				go srv.Run()
			case config.Echoing:
